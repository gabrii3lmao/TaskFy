...
describe("TasksService", () => {
    ...
    describe("Conclusão de Tarefa", () => {
        it("deve lançar erro 403 se um não-encarregado tentar concluir a tarefa", async () => {
            ...
            // EXECUÇÃO & VALIDAÇÃO
            await expect(TasksService.completeTask("task-1", "user-intruder")).rejects.toThrowError(new HttpException("Acesso negado: Apenas os encarregados pela tarefa podem concluí-la.", 403));
        });
    });
    describe("Cronômetro de Tarefa (Time Tracking)", () => {
        it("deve lançar erro 403 se um não-encarregado tentar dar play na tarefa", async () => {
            db.where.mockResolvedValueOnce([]); // Não é encarregado
            await expect(TasksService.startTaskTimer("task-1", "user-intruder")).rejects.toThrowError(new HttpException("Apenas encarregados podem iniciar a tarefa.", 403));
        });
        it("deve lançar erro 400 se o usuário já tiver um cronômetro rodando nesta tarefa", async () => {
            ...
            await expect(TasksService.startTaskTimer("task-1", "user-1")).rejects.toThrowError(new HttpException("Esta tarefa já possui um cronômetro ativo.", 400));
        });
        it("deve lançar erro 400 ao tentar pausar um cronômetro que não está rodando", async () => {
            ...
            db.where.mockResolvedValueOnce([]);
            await expect(TasksService.stopTaskTimer("task-1", "user-1")).rejects.toThrowError(new HttpException("Não há um cronômetro ativo para esta tarefa.", 400));
        });
    });
});
